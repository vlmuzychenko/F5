$(document).ready(function(){
  ...
  $('.hover_4').click(function(){
    ...
    openCard
    .to($('.card-front'), .3, {opacity:'0', display:'none', ease:Sine.easeOut})
    .to($('.main-card'), 0, {zIndex: '50'}, 0)

    .to($(this).siblings('.main-card'), .5, {scaleX:1, scaleY:1, height: '70vh', zIndex: '200'}, 0)

    .to($(this).parents('.card-container').find('.card-small'), 0, { opacity:'0', className:'-=active'}, 0)
    .to($('.hover_link').parents('.card-container').find('.card-small'), 0, {opacity:'1', className:'+=active'}, 0)

    .to($('.hover_link').parents('.card-container'), 1, {width:'30px', ease:Sine.easeOut}, 0)
    .to($(this).parents('.card-container'), 1, {width:'85%', ease:Power0.easeNone}, .5)

    .to($('.card-open'), 0, {className:'+=active'}, 0)

    .fromTo($(this).parents('.card-container').find('.card-open__img-wrap'), 1, {css:{transform:'scale(.8) translateX(-500px)', opacity:'0'}}, {css:{transform:'scale(1) translateX(0)', opacity:'1'}}, 1)

    .to($(this).parents('.card-container').find('.card-open__title'), 1, {opacity:'1'}, 1.3)

    .fromTo($('.hover_link').parents('.card-container').find('.main-card'), .5, {css:{transform:'translateZ(-100px) rotateY(-20deg)'}}, {css:{transform:'translateZ(0px) rotateY(0deg)'}, ease:Sine.easeOut}, 0)

    .to($(this).parents('.card-container').find('.card-open__items'), 0, {className:'+=active'}, 1.6)

    .to($(this).parents('.card-container').find('.card-open__close'), .3, {opacity:'1', display:'block', ease:Sine.easeOut});
  });
  ...
  $('.js-card-close').click(function(){
    ...
    closeCard
    .to($(this), .3, {opacity:'0', display:'none', ease:Sine.easeOut}, 0)
    .to($('.main-card'), 0, {zIndex: '50'}, 1.02)
    .to($('.card-open'), 1, {className:'-=active'}, 0)
    .to($('.card-open__img-wrap'), 1, {opacity:'0'}, 0)
    .to($('.card-open__title'), 0, {opacity:'0'}, 0)
    .to($('.card-open__items'), 0, {className:'-=active'}, 0)
    .to($('.card-small'), 1, {className:'-=active', opacity:'1'}, 0)
    .to($('.card-container'), 1, {width:'21.3%', ease:Power0.easeNone}, 0)
    .to($('.main-card'), 1, {width:'100%', ease:Power0.easeNone}, 0)
    .to($('.main-card'), 1, {height:'60vh', ease:Power0.easeNone}, 0)
    .to($('.card-container').find('.card-front'), .5, {opacity:'1', display:'block', ease:Sine.easeOut}, 1)
    .to($('.card-small'), 0, { opacity:'0'}, 1.01);
  });
...
$(document).click(function(e){
  if(!$('.popup__in').is(e.target) && !$('.btn_purple').is(e.target) && $('.popup__in').has(e.target).length === 0) {
    $('.popup').removeClass('is-open');
  }
